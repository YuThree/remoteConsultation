	...
	<div class="cdct_container cdct_right">
		<div class="cdct_content cdct_relative">
			...
			<div class="cdct_operation" style="margin-bottom:10px;">
				...
				<div class="y_inputBox">
					<div class="cdct_info"><span>科室选择</span></div>
					<el-select v-model="KS" clearable>
						<el-option v-for="(item,index) in KSArry" :key="index" :label="item.CKSMC" :value="item.CKSBH"></el-option>
					</el-select>
				</div>
			</div>
		...
		methods: {
			...
			initData() {
				this.$ajax.post('', {
					'MSH.1': 'sysPb',
					'MSH.2': 'SelCount',
					'IGHZL': localStorage.IGHZL, //系统编号
					'CKSBM': this.KS ? this.KS : '-1', //科室编码
					...
					if (re.data.rows&&re.data.rows.length>0) {
						...
						re.data.rows.forEach(element => {
							...
							let y = _this.doc.indexOf(element.CYSMC) * 2 + parseInt(element.ISJD) //data数组2层
...
<style lang="less" scoped>
	...
	.cdct_container {
		...
		.cdct_main {
			...
			.tableBg {
				// background: url("~@/assets/imgs/iconPaibanBg.png") no-repeat -194px 0;
			}
			.tabel-box {
				width: 100%;
				display: flex;

	...
	<div class="cdct_container cdct_right">
		<div class="cdct_content cdct_relative">
			...
			<div class="cdct_operation">
				...
				<div class="cdct_btn orange" @click="reset">重置</div>
			</div>
			<scroll :selectScroll="true" :useTransition="false" ref="scroll">
				<div class="cdct_main cdct_relative">
					...
					<div class="cdct_inputBox cdct_asterisk">
						...
						<input class="cdct_input" style="width: 98px; margin-right: 10px;" v-model="resObj.CXM" name="患者姓名" v-validate="'required'" disabled>
						<span class="cdct_error">{{verrors.first('患者姓名')}}</span>
						<div class="cdct_choose1" style="width: 60px;" @click="selectBtn">选择</div>
					</div>
				...
				<div class="cdct_main">
					...
					<div class="cdct_table spe_table" v-if="ICDArr.length > 0">
						...
						<div class="cdct_tr" v-for="(item,index) in ICDArr" :key="'jb'+index">
					...
					<div class="cdct_inputBox" style="width: 100%;">
						...
						<div class="cdct_choose choose2" @click="updata()">上传病例</div>
						<div class="Upload_Completed" v-show="isUpload"></div>
					</div>
				</div>
				<div class="cdct_main">
						...
						<div class="cdct_tr" v-for="(item,index) in hospitalArr" :key="'hzyy'+index">
							<div class="cdct_td cdct_relative">
								<el-select v-model="item.hospital" @change="changeHospital" v-validate="'required'" :data-vv-name="'上级会诊医院'+index" @visible-close="$validator.validate('上级会诊医院'+index)">
									<el-option v-for="(item,index) in SJJG" :key="index" :label="item.CMC" :value="item"></el-option>
								</el-select>
							...
							<div class="cdct_td cdct_relative">
								<el-select v-model="item.department" :disabled="!item.hospital" @change="changeKS(index,arguments)" v-validate="'required'" :data-vv-name="'会诊科室'+index" @visible-close="$validator.validate('会诊科室'+index)">
									<el-option v-for="(item,index) in HZKS" :key="index" :label="item.CKSMC" :value="item"></el-option>
								</el-select>
							...
							<div class="cdct_td">
								<div class="info_check">
									...
									<span @click="item.department != '' && selectDoc(item.department.CKSBH,index)"></span>
								</div>
							</div>
				...
				<div class="cdct_main">
					...
					<div class="cdct_inputBox">
						...
						<input class="cdct_input" disabled value="测试" v-model="loginInfo.CJGMC">
					</div>
					<div class="cdct_inputBox">
						<div class="cdct_info"><span>申请科室</span></div>
						<input class="cdct_input" disabled v-model="loginInfo.CKSMC">
					</div>
					<div class="cdct_inputBox">
						<div class="cdct_info"><span>申请医生</span></div>
						<input class="cdct_input" disabled v-model="loginInfo.CYSMC">
					</div>
				</div>
			</scroll>
		</div>
		<select-modal v-model="selectPatient" width="808px" height="459px" @search="submit" :pullUpLoad="true" @pullingUp="patientPullingUp" ref="patientModal">
			...
			<div class="tbody" slot="body">
				<dl class="tb-tr" v-for="(item,index) in patientArr" :key="'patient'+index" @click="updatePatient(item)">
					<dd class="tb-td"><span>{{item.CJGMC}}</span></dd>
					<dd class="tb-td"><span>{{item.CBRXM}}</span></dd>
					<dd class="tb-td" style="flex: 0.5;"><span>{{item.CBRXBMC}}</span></dd>
					<dd class="tb-td" style="flex: 0.5;"><span>{{getAge(item.DCSNY)}}</span></dd>
					<dd class="tb-td" style="flex: 1.5;"><span>{{item.CZJHM}}</span></dd>
					<dd class="tb-td"><span>{{item.CHZDH}}</span></dd>
					<dd class="tb-td"><span>{{getPatientType(item.IBRXZ)}}</span></dd>
				</dl>
			</div>
		</select-modal>
...
<script>
	...
	export default {
		...
		updata(){
			if(resObj.CBRID){
				$router.push({name:'uploadTheCases', params:{resObj:resObj,systemType:'1'}});
			}else{
				this.$MessageCase.show('请选择患者', 'error');
			}
		},
		activated() {
			this.$store.commit('setIncludeds', 'NewApplication'); //此处设置name一致的名称
		},
		...
		beforeRouteLeave(to, from, next) {
			...
			if (!addPage.includes(to.name)) {
				this.$store.commit('setIncludeds', '');
			}
			next();
		},
		mounted() {
		...
		methods: {
			getCBH() {
				...
				this.$ajax.post('', {
					...
					if (res.code && res.code == '100') {
						this.resObj.AddCBH = res.data.SQDBH;
					}
				}).catch(error => {
					console.log(error)
				});
			...
			updatePatient(item) {
				...
				this.$set(this.resObj, 'CBRID', item.CBRID); // 患者ID 
				this.$set(this.resObj, 'CLXDH', item.CHZDH); // 患者电话
			...
			getHospital() { // 获取上级机构
				this.$ajax.post('', {
					...
					if (res.code == '100') {
						this.SJJG = this.SJJG.concat(res.data);
					}
				}).catch(error => {
					console.log(error)
				});
			...
			getHZKS(str) { // 获取会诊科室
				this.$ajax.post('', {
					...
					if (res.code == '100') {
						this.HZKS = this.HZKS.concat(res.data.rows);
					}
				}).catch(error => {
					console.log(error)
				});
					...
					if (result) {
						...
						this.hospitalArr.forEach(item => {
							if (item.doctor.CYSBM) {
								stringDataArr.push(`${item.hospital.CBH}-${item.hospital.CMC},${item.department.CKSBH}-${item.department.CKSMC},${item.doctor.CYSBM}-${item.doctor.CYSXM}`)
							} else {
								stringDataArr.push(`${item.hospital.CBH}-${item.hospital.CMC},${item.department.CKSBH}-${item.department.CKSMC},0-医生为空`);
							}
						});
						// 身份证号

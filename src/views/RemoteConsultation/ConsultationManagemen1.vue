					...
					<div class="y_cardBox " v-for="(i,index) in data" :key="index" :class="{'pendingAudit':i.IZT == 1,'refuse':i.IZT == 2,'cancel':i.IZT == 3,'wConsultation':i.IZT == 4,'diagnosis':i.IZT == 5,'over':i.IZT == 6,'emergency':i.ILX == 1}">
						...
						<div class="y_chooseBtn">
							...
							<message-box :ref="'mes'+index" offsetLeft="-165px" offsetTop="-38px" width="156px" maxHeight="250px">
								<div class="y_ctrlBox">
									<!-- 
													进入会诊室（IZT=4或IZT=5）[并且IHZFS=1(即为交互方式)]----120001
													查看会诊申请：
														申请管理-----110001
														待会诊管理----120002
													查看病历：
													...
													编辑会诊申请：(IZT=1)------110002
													取消会诊申请：(IZT=1)------110003
													查看诊断报告: (IZT=6[并且BGDSL不为空])----120004
													填写会诊评价：(IZT=6[并且PJSL不为空])-----120005
													查看会诊评价：(IZT=6[并且PJSL为空])-------120006
												 -->
									...
									<span class="jrhzs" @click="$refs['mes'+index][0].show();tomeetingRoom(i)" v-show="getBtnIn('120001')&&i.IHZFS == 1&&(i.IZT == 4||i.IZT == 5)">进入会诊室</span>
									<span class="txpj" @click="$router.push({name:'ServiceEvaluation',params:{info:i}})" v-show="getBtnIn('120005')&&i.IZT == 6&&i.PJSL==''">填写评价</span>
									<span class="ckpj" @click="$router.push({name:'ServiceEvaluation',params:{info:i}})" v-show="getBtnIn('120006')&&i.IZT == 6&&i.PJSL!=''">查看评价</span>
									<span class="ckhzbg" @click="$router.push({name:'FillDiagnosis',params:{info:i,look:true}})" v-show="getBtnIn('120004')&&i.IZT == 6&&i.BGDSL!=''">查看会诊报告</span>
								</div>
							</message-box>
						</div>

			...
			<scroll :selectScroll="true" :useTransition="false" >
				...
				<div class="cdct_main cdct_relative">
					...
					<div class="cdct_inputBox cdct_asterisk">
						...
						<input class="cdct_input" name="检查单号" v-validate="'required'">
						<span class="cdct_error">{{verrors.first('检查单号')}}</span>
					</div>
				...
				<div class="cdct_main">
					...
					<div class="cdct_inputBox cdct_asterisk">
						...
						<datetime class="cdct_time" :min-year="1900" :start-date="getNowFormatDate()" v-model="TEST" v-validate="'required'" data-vv-name="诊断日期" @on-hide="$validator.validate('诊断日期')" ></datetime>
						<span class="cdct_error">{{verrors.first('诊断日期')}}</span>
					</div>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			params.append('username', 'ychzsq');       //你要传给后台的参数值 key/value
